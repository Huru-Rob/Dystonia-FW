...
typedef enum {
    ...
    store_reason_bad_crc = 8
} store_reason_e;

// This structure maintains compatibility with PT10A in CLIC form
typedef struct __attribute__ ((__packed__)) {
    uint32_t device_id;         // 4
    uint32_t deep_sleep_flag;   // 8
    uint32_t store_reason;      // 12
    uint32_t bluetooth_mode;    // 16 particularly - keep this and set it to one to ensure that bluetooth comes up if re-programmed to be a CLIC device
    int32_t frequency;          // 20
    int32_t trigger_phase;      // 24
    int32_t suppression_duty_cycle; // 28
    int32_t convergence_gain;   // 32
    uint32_t flags;             // 36
    int16_t phases[NEUROBUZZ_MAX_PHASES]; // 48
    uint8_t num_phases;         // 49
    uint32_t long_dur;          // 53
    uint32_t short_dur;         // 57
    uint32_t switch_stim_gap;   // 61
    int16_t step_near_preferred_phase; // 63
    uint8_t disable_refinement; // 64
    uint8_t unused[3];            // 67
    uint8_t crc;                // 68
} nv_params_t;
